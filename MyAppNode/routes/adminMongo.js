	...
	db.open(function(err, db) { }); // end of db.open
				...
				oracle.connect(connectData, function(err, connection) {
				    if ( err ) {
				    	...
				    	var query="select OBJECT_ID, url, count from OBJECTS where OBJECTS.count>=5";
...
var saveAllToMongo= function(cachedList, callback){
    ...
    // Loop for each image, check if it is on Mongo. If not, then store it. 
    	...
    	imageName = cachedList[i].OBJECT_ID + imageType;
    	imageUrl = cachedList[i].URL;
    	objectID = cachedList[i].OBJECT_ID;
        	...
        	flowController.on('doWork', function (i) {
        		...
        		asnycfunction(objectID,imageUrl, imageName,imageType, db, function(){
        			if (i<cachedList.length){
        			imageType = cachedList[i].URL.charAt(cachedList[i].URL.length-3) + cachedList[i].URL.charAt(cachedList[i].URL.length-2) +cachedList[i].URL.charAt(cachedList[i].URL.length-1);
        	    	imageName = cachedList[i].OBJECT_ID + imageType;
        	    	imageUrl = cachedList[i].URL;
...
var asnycfunction = function(objectID,imageUrl, imageName, imageType, db, callback){
...
var setcached = function(objID, callback)
{
	...
	console.log(objID);
		...
		oracle.connect(connectData, function(err, connection) {
		    if ( err ) {
		    	...
		    	var query="update objects set iscached='T' where object_id='" + objID + "' ";
